...
// BFS traversal and label the nodes
    ...
    {
// v<q> = level
        GrB_Vector_assign_BOOL(v, q, nullptr, level, GrB_ALL, n, GrB_DESC_S) ;
// break if q is empty
        GrB_Vector_nvals (&nvals, q) ;
        if (nvals == 0) break ;
// q’<!v> = q’*A
        GrB_vxm (q, v, nullptr, GxB_ANY_PAIR_BOOL, q, A, GrB_DESC_RC) ;
    }
// free workspace and return result
    (*s) = v ; // return result
    v = nullptr ; // set to NULL so LAGRAPH_FREE_ALL doesn’t free it
    LAGRAPH_FREE_ALL;
    return (GrB_SUCCESS) ;
}
